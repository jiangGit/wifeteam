...
function loadMusic(musicList,callback)
{
	...
	Game.Data.CountLoad.totalMusic=musicList.length;;
	var loadedCount=0;
	for (var i=0;i<musicsCount;i++){
		...
		if(isSupportAudio()){
			...
			loadedCount++;		
			Game.Data.CountLoad.loadedMusic=loadedCount;
		}
...
function checkPointInObj(obj,e){
	...
	var x = e.clientX - (obj.x+Game.Container.zoffsetLeft);
	var y = e.clientY - (obj.y+Game.Container.zoffsetTop);
...
function drawLine(x,y,vx,vy,g){
	...
	while(true){
		...
		if(t >= Game.Container.height) break;			
			...
			cxt.beginPath();
			cxt.arc(s,t+y,5,0,Math.PI*2,true);
			cxt.closePath();
			cxt.fill();
			s+=20;
	}
